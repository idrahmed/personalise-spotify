...
function TopArtists({ user, userName, useruri }) {
  const [top_artists, setArtists] = useState(null);
  ...
  const addToSaved = (id) => {
    axios
      ...
      .then(
        alert.show(<div style={{ fontSize: "14px" }}>Followed Artist</div>)
      );
  };
      ...
      >
        <div className="body">
          ...
          <ul className="list">
            ...
            {top_artists?.length === 0 && <h1 className="no_data">No data to display</h1>}
